...
export default function ElectronicsForm({ selectedSubCategory }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        if (key === "features") {
          // Convert features array to JSON string as expected by API
      ...
      console.log("[v0] Submitting electronics form data to API...");

      const response = await fetch(
        `https://sellit-backend-u8bz.onrender.com/api/ads/electronics/${uid}`,
        {
          method: "POST",
          body: data,
        }
      ...
      console.log("✅ Electronics ad created:", result);
      ...
      if (err.message.includes("fetch")) {
        ...
        alert(`Error submitting electronics ad: ${err.message}`);
      }
    } finally {
